...
;(() => {
  ...
  let mmdData: MMDData | null = null  // 最终解压处理好的mmd数据
  
  // 拦截请求
  self.addEventListener('fetch', (e: any) => {
    // 注意respondWith不能异步执行，有异步流程的要提前处理
    ...
    if (!(pmxPath in mmdData)) { return }

    e.respondWith(new Response(mmdData[pmxPath]))    
  })
  ...
  self.addEventListener('message', e => {
    ...
    bindMsgHandler('zipReady', async data => {
      console.log(data)
      const unzippedData = await unzip(data.file)
      ...
      mmdData = mmdBlobs
      const pmxFileName = Object.keys(mmdData!).find(item => /\.pmx$/.test(item))!
      // mmd数据准备完毕，通知主线程
